...
int main()
{
    ...
    in.on_change(std::bind(&StaticGrid::resizew, std::ref(grid), std::placeholders::_1));
    seq.add(&in);
    ...
    shrink_in.on_change(std::bind(&Button::resizew, std::ref(button), std::placeholders::_1));
    experimental::PropertyAnimator shrink_out(10, 100, std::chrono::seconds(1));
    shrink_out.on_change(std::bind(&Button::resizew, std::ref(button), std::placeholders::_1));
    seq.add(&shrink_in);
    seq.add(&shrink_out);
    ...
    shrinkh_in.on_change(std::bind(&Label::resizew, std::ref(label), std::placeholders::_1));
    experimental::PropertyAnimator shrinkh_out(10, 100, std::chrono::seconds(1));
    shrinkh_out.on_change(std::bind(&Label::resizew, std::ref(label), std::placeholders::_1));
    seq.add(&shrinkh_in);
    seq.add(&shrinkh_out);
