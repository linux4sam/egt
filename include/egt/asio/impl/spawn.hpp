...
#ifndef EGT_ASIO_IMPL_SPAWN_HPP
#define EGT_ASIO_IMPL_SPAWN_HPP
...
namespace detail {
  ...
  public:
    ...
    void operator()(T value)
    {
      ...
      *value_ = EGT_ASIO_MOVE_CAST(T)(value);
      if (--*ready_ == 0)
        (*coro_)();
    }

    void operator()(egt::asio::error_code ec, T value)
    {
      *ec_ = ec;
      *value_ = EGT_ASIO_MOVE_CAST(T)(value);
      if (--*ready_ == 0)
  ...
  public:
    ...
    void operator()(egt::asio::error_code ec)
    {
      *ec_ = ec;
      if (--*ready_ == 0)
        (*coro_)();
  ...
  public:
    ...
    return_type get()
    {
      ...
      if (--ready_ != 0)
        ca_();
      if (!out_ec_ && ec_) throw egt::asio::system_error(ec_);
      return EGT_ASIO_MOVE_CAST(return_type)(value_);
    }
...
class async_result<basic_yield_context<Handler>,
    ReturnType(egt::asio::error_code)>
  : public detail::coro_async_result<Handler, void>
{
public:
  explicit async_result(
...
class async_result<basic_yield_context<Handler>,
    ReturnType(egt::asio::error_code, Arg2)>
...
namespace detail {
  ...
  struct spawn_data : private noncopyable
  {
    ...
    spawn_data(EGT_ASIO_MOVE_ARG(Hand) handler,
        bool call_handler, EGT_ASIO_MOVE_ARG(Func) function)
      : handler_(EGT_ASIO_MOVE_CAST(Hand)(handler)),
        call_handler_(call_handler),
        function_(EGT_ASIO_MOVE_CAST(Func)(function))
    {
    }
...
inline void spawn(EGT_ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes& attributes)
{
  ...
  egt::asio::spawn(ex, EGT_ASIO_MOVE_CAST(Function)(function), attributes);
}

template <typename Handler, typename Function>
void spawn(EGT_ASIO_MOVE_ARG(Handler) handler,
    EGT_ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes& attributes,
      ...
      !is_convertible<Handler&, execution_context&>::value>::type*)
{
  ...
  helper.data_.reset(
      new detail::spawn_data<handler_type, function_type>(
        EGT_ASIO_MOVE_CAST(Handler)(handler), true,
        EGT_ASIO_MOVE_CAST(Function)(function)));
    ...
    const boost::coroutines::attributes& attributes)
{
  ...
  helper.data_.reset(
      new detail::spawn_data<Handler, function_type>(
        EGT_ASIO_MOVE_CAST(Handler)(handler), false,
        EGT_ASIO_MOVE_CAST(Function)(function)));
  helper.attributes_ = attributes;
...
{
  egt::asio::spawn(egt::asio::strand<Executor>(ex),
      EGT_ASIO_MOVE_CAST(Function)(function), attributes);
}
...
inline void spawn(const strand<Executor>& ex,
    ...
    const boost::coroutines::attributes& attributes)
{
  egt::asio::spawn(egt::asio::bind_executor(
        ex, &detail::default_spawn_handler),
      EGT_ASIO_MOVE_CAST(Function)(function), attributes);
}

template <typename Function>
inline void spawn(const egt::asio::io_context::strand& s,
    EGT_ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes& attributes)
{
  egt::asio::spawn(egt::asio::bind_executor(
...
{
  egt::asio::spawn(ctx.get_executor(),
      EGT_ASIO_MOVE_CAST(Function)(function), attributes);
}
