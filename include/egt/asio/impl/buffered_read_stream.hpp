...
#ifndef EGT_ASIO_IMPL_BUFFERED_READ_STREAM_HPP
#define EGT_ASIO_IMPL_BUFFERED_READ_STREAM_HPP
...
template <typename Stream>
std::size_t buffered_read_stream<Stream>::fill(egt::asio::error_code& ec)
{
  detail::buffer_resize_guard<detail::buffered_stream_storage>
    resize_guard(storage_);
  std::size_t previous_size = storage_.size();
    ...
    EGT_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  next_layer_.async_read_some(
      buffer(
        storage_.data() + previous_size,
        storage_.size() - previous_size),
      detail::buffered_fill_handler<EGT_ASIO_HANDLER_TYPE(
    ...
    EGT_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  {
    next_layer_.async_read_some(EGT_ASIO_MUTABLE_BUFFER(0, 0),
        detail::buffered_read_some_handler<
          MutableBufferSequence, EGT_ASIO_HANDLER_TYPE(
            ReadHandler, void (egt::asio::error_code, std::size_t))>(
...
#endif // EGT_ASIO_IMPL_BUFFERED_READ_STREAM_HPP
