...
#ifndef EGT_ASIO_IMPL_BUFFERED_WRITE_STREAM_HPP
#define EGT_ASIO_IMPL_BUFFERED_WRITE_STREAM_HPP
...
template <typename Stream>
std::size_t buffered_write_stream<Stream>::flush(egt::asio::error_code& ec)
{
  std::size_t bytes_written = write(next_layer_,
      buffer(storage_.data(), storage_.size()),
      transfer_all(), ec);
    ...
    EGT_ASIO_MOVE_ARG(WriteHandler) handler)
{
  ...
  async_write(next_layer_, buffer(storage_.data(), storage_.size()),
      detail::buffered_flush_handler<EGT_ASIO_HANDLER_TYPE(
...
namespace detail
{
  ...
  public:
    ...
    void operator()(const egt::asio::error_code& ec, std::size_t)
    {
      if (ec)
      {
      ...
      else
      {
        ...
        const std::size_t bytes_copied = egt::asio::buffer_copy(
    ...
    EGT_ASIO_MOVE_ARG(WriteHandler) handler)
{
  ...
  {
    next_layer_.async_write_some(EGT_ASIO_CONST_BUFFER(0, 0),
        detail::buffered_write_some_handler<
          ConstBufferSequence, EGT_ASIO_HANDLER_TYPE(
            WriteHandler, void (egt::asio::error_code, std::size_t))>(
...
#endif // EGT_ASIO_IMPL_BUFFERED_WRITE_STREAM_HPP
