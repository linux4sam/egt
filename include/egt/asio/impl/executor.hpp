...
#ifndef EGT_ASIO_IMPL_EXECUTOR_HPP
#define EGT_ASIO_IMPL_EXECUTOR_HPP
...
public:
  ...
  explicit function(F f, const Alloc& a)
  {
    ...
    op_ = new (p.v) op(EGT_ASIO_MOVE_CAST(F)(f), a);
    p.v = 0;
  }
  ...
  void operator()()
  {
    if (op_)
    {
      ...
      op_ = 0;
      op->complete(this, egt::asio::error_code(), 0);
    }
  }
...
{
public:
  typedef EGT_ASIO_REBIND_ALLOC(Allocator, impl) allocator_type;
  ...
  impl(const Executor& e, const Allocator& a) EGT_ASIO_NOEXCEPT
    : impl_base(false),
      ref_count_(1),
      executor_(e),
      allocator_(a)
  ...
  void on_work_started() EGT_ASIO_NOEXCEPT
  {
    executor_.on_work_started();
  }

  void on_work_finished() EGT_ASIO_NOEXCEPT
  {
    executor_.on_work_finished();
  }

  execution_context& context() EGT_ASIO_NOEXCEPT
  {
    return executor_.context();
  }

  void dispatch(EGT_ASIO_MOVE_ARG(function) f)
  {
    executor_.dispatch(EGT_ASIO_MOVE_CAST(function)(f), allocator_);
  }

  void post(EGT_ASIO_MOVE_ARG(function) f)
  {
    executor_.post(EGT_ASIO_MOVE_CAST(function)(f), allocator_);
  }

  void defer(EGT_ASIO_MOVE_ARG(function) f)
  {
    executor_.defer(EGT_ASIO_MOVE_CAST(function)(f), allocator_);
  }

  type_id_result_type target_type() const EGT_ASIO_NOEXCEPT
  {
    return type_id<Executor>();
  }

  void* target() EGT_ASIO_NOEXCEPT
  {
    return &executor_;
  }

  const void* target() const EGT_ASIO_NOEXCEPT
  {
    return &executor_;
  }

  bool equals(const impl_base* e) const EGT_ASIO_NOEXCEPT
  {
    if (this == e)
      return true;
...
void executor::dispatch(EGT_ASIO_MOVE_ARG(Function) f,
    const Allocator& a) const
{
  impl_base* i = get_impl();
  if (i->fast_dispatch_)
    system_executor().dispatch(EGT_ASIO_MOVE_CAST(Function)(f), a);
  else
    i->dispatch(function(EGT_ASIO_MOVE_CAST(Function)(f), a));
}

template <typename Function, typename Allocator>
void executor::post(EGT_ASIO_MOVE_ARG(Function) f,
    const Allocator& a) const
{
  get_impl()->post(function(EGT_ASIO_MOVE_CAST(Function)(f), a));
}

template <typename Function, typename Allocator>
void executor::defer(EGT_ASIO_MOVE_ARG(Function) f,
    const Allocator& a) const
{
  get_impl()->defer(function(EGT_ASIO_MOVE_CAST(Function)(f), a));
}

template <typename Executor>
Executor* executor::target() EGT_ASIO_NOEXCEPT
{
...
template <typename Executor>
const Executor* executor::target() const EGT_ASIO_NOEXCEPT
{
  return impl_ && impl_->target_type() == type_id<Executor>()
    ? static_cast<Executor*>(impl_->target()) : 0;
}
...
#endif // EGT_ASIO_IMPL_EXECUTOR_HPP
