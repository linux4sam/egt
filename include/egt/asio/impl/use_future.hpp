...
#ifndef EGT_ASIO_IMPL_USE_FUTURE_HPP
#define EGT_ASIO_IMPL_USE_FUTURE_HPP
...
inline void promise_invoke_and_set(std::promise<T>& p,
    F& f, EGT_ASIO_MOVE_ARG(Args)... args)
{
#if !defined(EGT_ASIO_NO_EXCEPTIONS)
  try
#endif // !defined(EGT_ASIO_NO_EXCEPTIONS)
  {
    p.set_value(f(EGT_ASIO_MOVE_CAST(Args)(args)...));
  }
#if !defined(EGT_ASIO_NO_EXCEPTIONS)
  catch (...)
  {
...
#endif // !defined(EGT_ASIO_NO_EXCEPTIONS)
  {
    f(EGT_ASIO_MOVE_CAST(Args)(args)...);
    p.set_value();
  }
#if !defined(EGT_ASIO_NO_EXCEPTIONS)
  catch (...)
...
#define EGT_ASIO_PRIVATE_PROMISE_INVOKE_DEF(n) \
  template <typename T, typename F, EGT_ASIO_VARIADIC_TPARAMS(n)> \
  inline void promise_invoke_and_set(std::promise<T>& p, \
      F& f, EGT_ASIO_VARIADIC_MOVE_PARAMS(n)) \
  { \
    p.set_value(f(EGT_ASIO_VARIADIC_MOVE_ARGS(n))); \
  } \
  \
  template <typename F, EGT_ASIO_VARIADIC_TPARAMS(n)> \
  inline void promise_invoke_and_set(std::promise<void>& p, \
      F& f, EGT_ASIO_VARIADIC_MOVE_PARAMS(n)) \
  { \
    f(EGT_ASIO_VARIADIC_MOVE_ARGS(n)); \
    p.set_value(); \
  } \
  /**/
  EGT_ASIO_VARIADIC_GENERATE(EGT_ASIO_PRIVATE_PROMISE_INVOKE_DEF)
#undef EGT_ASIO_PRIVATE_PROMISE_INVOKE_DEF

#else // defined(EGT_ASIO_NO_EXCEPTIONS)
...
public:
  promise_invoker(const shared_ptr<std::promise<T> >& p,
      EGT_ASIO_MOVE_ARG(F) f)
    : p_(p), f_(EGT_ASIO_MOVE_CAST(F)(f))
  {
  }
...
public:
  ...
  execution_context& context() const EGT_ASIO_NOEXCEPT
  {
    return system_executor().context();
  }

  void on_work_started() const EGT_ASIO_NOEXCEPT {}
  void on_work_finished() const EGT_ASIO_NOEXCEPT {}

  template <typename F, typename A>
  void dispatch(EGT_ASIO_MOVE_ARG(F) f, const A&) const
  {
    promise_invoker<T, F>(p_, EGT_ASIO_MOVE_CAST(F)(f))();
  }

  template <typename F, typename A>
  void post(EGT_ASIO_MOVE_ARG(F) f, const A& a) const
  {
    system_executor().post(
        promise_invoker<T, F>(p_, EGT_ASIO_MOVE_CAST(F)(f)), a);
  }

  template <typename F, typename A>
  void defer(EGT_ASIO_MOVE_ARG(F) f, const A& a) const
  {
  ...
  friend bool operator==(const promise_executor& a,
      const promise_executor& b) EGT_ASIO_NOEXCEPT
  {
    return a.p_ == b.p_;
  }
...
public:
  ...
  executor_type get_executor() const EGT_ASIO_NOEXCEPT
  {
...
protected:
  ...
  {
    EGT_ASIO_REBIND_ALLOC(Allocator, char) b(a);
...
public:
  ...
  void operator()(EGT_ASIO_MOVE_ARG(Arg) arg)
  {
    this->p_->set_value(EGT_ASIO_MOVE_CAST(Arg)(arg));
  }
};
...
public:
  ...
  void operator()(EGT_ASIO_MOVE_ARG(Args)... args)
  {
    this->p_->set_value(
...
#define EGT_ASIO_PRIVATE_CALL_OP_DEF(n) \
  template <EGT_ASIO_VARIADIC_TPARAMS(n)> \
  void operator()(EGT_ASIO_VARIADIC_MOVE_PARAMS(n)) \
  {\
    this->p_->set_value( \
        std::forward_as_tuple( \
  ...
  /**/
  EGT_ASIO_VARIADIC_GENERATE(EGT_ASIO_PRIVATE_CALL_OP_DEF)
#undef EGT_ASIO_PRIVATE_CALL_OP_DEF
...
template <>
class promise_handler_selector<void(egt::asio::error_code)>
...
template <typename Arg>
class promise_handler_selector<void(egt::asio::error_code, Arg)>
...
#define EGT_ASIO_PRIVATE_PROMISE_SELECTOR_DEF(n) \
  ...
  class promise_handler_selector< \
    void(Arg, EGT_ASIO_VARIADIC_TARGS(n))> \
      : public promise_handler_n< \
        std::tuple<Arg, EGT_ASIO_VARIADIC_TARGS(n)> > {}; \
  \
  template <typename Arg, EGT_ASIO_VARIADIC_TPARAMS(n)> \
  class promise_handler_selector< \
    void(egt::asio::error_code, Arg, EGT_ASIO_VARIADIC_TARGS(n))> \
      : public promise_handler_ec_n< \
        std::tuple<Arg, EGT_ASIO_VARIADIC_TARGS(n)> > {}; \
  \
  template <typename Arg, EGT_ASIO_VARIADIC_TPARAMS(n)> \
  class promise_handler_selector< \
    void(std::exception_ptr, Arg, EGT_ASIO_VARIADIC_TARGS(n))> \
      : public promise_handler_ex_n< \
        std::tuple<Arg, EGT_ASIO_VARIADIC_TARGS(n)> > {}; \
  /**/
  EGT_ASIO_VARIADIC_GENERATE(EGT_ASIO_PRIVATE_PROMISE_SELECTOR_DEF)
#undef EGT_ASIO_PRIVATE_PROMISE_SELECTOR_DEF
...
public:
  ...
  allocator_type get_allocator() const EGT_ASIO_NOEXCEPT
  {
    return allocator_;
  }
    ...
    promise_handler<Signature, Allocator>* h)
{
  typename promise_handler<Signature, Allocator>::executor_type
    ex(h->get_executor());
  ex.dispatch(EGT_ASIO_MOVE_CAST(Function)(f), std::allocator<void>());
}
...
public:
  ...
  return_type get()
  {
    return EGT_ASIO_MOVE_CAST(return_type)(future_);
  }
...
public:
  packaged_token(Function f, const Allocator& a)
    : function_(EGT_ASIO_MOVE_CAST(Function)(f)),
      allocator_(a)
  {
  }
...
public:
  ...
  packaged_handler(packaged_token<Function, Allocator> t)
    : function_(EGT_ASIO_MOVE_CAST(Function)(t.function_)),
  ...
  void operator()(EGT_ASIO_MOVE_ARG(Args)... args)
  {
    (promise_invoke_and_set)(*this->p_,
        function_, EGT_ASIO_MOVE_CAST(Args)(args)...);
  }
...
#define EGT_ASIO_PRIVATE_CALL_OP_DEF(n) \
  ...
  {\
    (promise_invoke_and_set)(*this->p_, \
        function_, EGT_ASIO_VARIADIC_MOVE_ARGS(n)); \
  } \
  /**/
  EGT_ASIO_VARIADIC_GENERATE(EGT_ASIO_PRIVATE_CALL_OP_DEF)
#undef EGT_ASIO_PRIVATE_CALL_OP_DEF
...
use_future_t<Allocator>::operator()(EGT_ASIO_MOVE_ARG(Function) f) const
{
  return detail::packaged_token<typename decay<Function>::type, Allocator>(
      EGT_ASIO_MOVE_CAST(Function)(f), allocator_);
}
...
#define EGT_ASIO_PRIVATE_ASYNC_RESULT_DEF(n) \
  template <typename Allocator, \
      typename Result, EGT_ASIO_VARIADIC_TPARAMS(n)> \
  class async_result<use_future_t<Allocator>, \
      Result(EGT_ASIO_VARIADIC_TARGS(n))> \
    : public detail::promise_async_result< \
        void(EGT_ASIO_VARIADIC_DECAY(n)), Allocator> \
  { \
  public: \
    explicit async_result( \
      typename detail::promise_async_result< \
        void(EGT_ASIO_VARIADIC_DECAY(n)), \
        Allocator>::completion_handler_type& h) \
      : detail::promise_async_result< \
          void(EGT_ASIO_VARIADIC_DECAY(n)), Allocator>(h) \
    { \
    } \
  }; \
  \
  ...
  class async_result<detail::packaged_token<Function, Allocator>, \
      Result(EGT_ASIO_VARIADIC_TARGS(n))> \
    : public detail::packaged_async_result<Function, Allocator, \
        typename result_of<Function(EGT_ASIO_VARIADIC_TARGS(n))>::type> \
  { \
  public: \
    explicit async_result( \
      typename detail::packaged_async_result<Function, Allocator, \
      ...
      : detail::packaged_async_result<Function, Allocator, \
          typename result_of<Function(EGT_ASIO_VARIADIC_TARGS(n))>::type>(h) \
    { \
    } \
  }; \
  /**/
...
#endif // EGT_ASIO_IMPL_USE_FUTURE_HPP
