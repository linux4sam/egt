...
#ifndef EGT_ASIO_BASIC_WAITABLE_TIMER_HPP
#define EGT_ASIO_BASIC_WAITABLE_TIMER_HPP
...
# include <egt/asio/detail/chrono_time_traits.hpp>
...
#if !defined(EGT_ASIO_BASIC_WAITABLE_TIMER_FWD_DECL)
#define EGT_ASIO_BASIC_WAITABLE_TIMER_FWD_DECL

// Forward declaration with defaulted arguments.
template <typename Clock,
    typename WaitTraits = egt::asio::wait_traits<Clock>
    EGT_ASIO_SVC_TPARAM_DEF2(= waitable_timer_service<Clock, WaitTraits>)>
class basic_waitable_timer;

#endif // !defined(EGT_ASIO_BASIC_WAITABLE_TIMER_FWD_DECL)
 ...
 *
 * Most applications will use one of the egt::asio::steady_timer,
 * egt::asio::system_timer or egt::asio::high_resolution_timer typedefs.
 *
 ...
 * egt::asio::steady_timer timer(io_context);
 *
 ...
 * @code
 * void handler(const egt::asio::error_code& error)
 * {
 *   if (!error)
 *   {
 *     // Timer expired.
 ...
 * }
 *
 * void on_timeout(const egt::asio::error_code& e)
 * {
 *   if (e != egt::asio::error::operation_aborted)
 *   {
 ...
 * @endcode
 *
 * @li The egt::asio::basic_waitable_timer::expires_after() function
 * cancels any pending asynchronous waits, and returns the number of
 * asynchronous waits that were cancelled. If it returns 0 then you were too
 * late and the wait handler has already been executed, or will soon be
 * executed. If it returns 1 then the wait handler was successfully cancelled.
 *
 * @li If a wait handler is cancelled, the egt::asio::error_code passed to
 * it contains the value egt::asio::error::operation_aborted.
 */
template <typename Clock, typename WaitTraits EGT_ASIO_SVC_TPARAM>
class basic_waitable_timer
...
public:
   ...
   */
  explicit basic_waitable_timer(egt::asio::io_context& io_context)
    ...
    : basic_io_object<EGT_ASIO_SVC_T>(io_context)
  {
    ...
    egt::asio::detail::throw_error(ec, "expires_after");
  }
   ...
   * be invoked with the egt::asio::error::operation_aborted error code.
   *
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel(egt::asio::error_code& ec)
  {
   ...
   * egt::asio::error::operation_aborted error code.
   *
   * Cancelling the timer does not change the expiry time.
   *
  ...
  std::size_t cancel_one()
  {
    ...
    egt::asio::detail::throw_error(ec, "cancel_one");
    return s;
  }
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel_one(egt::asio::error_code& ec)
  {
  ...
  std::size_t expires_from_now(const duration& expiry_time)
  {
    ...
    std::size_t s = this->get_service().expires_from_now(
        this->get_implementation(), expiry_time, ec);
    egt::asio::detail::throw_error(ec, "expires_from_now");
    return s;
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  void wait(egt::asio::error_code& ec)
  {
    this->get_service().wait(this->get_implementation(), ec);
  }
   ...
   * code egt::asio::error::operation_aborted.
   *
  ...
  async_wait(EGT_ASIO_MOVE_ARG(WaitHandler) handler)
  {
    ...
    return this->get_service().async_wait(this->get_implementation(),
        EGT_ASIO_MOVE_CAST(WaitHandler)(handler));
#else // defined(EGT_ASIO_ENABLE_OLD_SERVICES)
...
private:
  ...
  basic_waitable_timer(const basic_waitable_timer&) EGT_ASIO_DELETED;
  basic_waitable_timer& operator=(
      const basic_waitable_timer&) EGT_ASIO_DELETED;
};
...
#endif // EGT_ASIO_BASIC_WAITABLE_TIMER_HPP
