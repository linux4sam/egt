...
#ifndef EGT_ASIO_CONNECT_HPP
#define EGT_ASIO_CONNECT_HPP
...
/**
 * @defgroup connect egt::asio::connect
 *
 ...
 *
 * @throws egt::asio::system_error Thrown on failure. If the sequence is
 * empty, the associated @c error_code is egt::asio::error::not_found.
 ...
 * egt::asio::connect(s, r.resolve(q)); @endcode
 */
 ...
 * empty, set to egt::asio::error::not_found. Otherwise, contains the error
 ...
 * egt::asio::connect(s, r.resolve(q), ec);
 * if (ec)
 * {
 *   // An error occurred.
 * } @endcode
 ...
 * iterator types such as @c egt::asio::ip::tcp::resolver::iterator.
 */
 ...
 * tcp::socket s(io_context);
 * egt::asio::connect(s, e.begin(), e.end()); @endcode
 */
template <typename Protocol EGT_ASIO_SVC_TPARAM, typename Iterator>
Iterator connect(basic_socket<Protocol EGT_ASIO_SVC_TARG>& s,
 ...
 * tcp::socket s(io_context);
 * egt::asio::error_code ec;
 * egt::asio::connect(s, e.begin(), e.end(), ec);
 * if (ec)
 * {
 *   // An error occurred.
 * } @endcode
 ...
 *   }
 * }; @endcode
 * It would be used with the egt::asio::connect function as follows:
 * @code tcp::resolver r(io_context);
 * tcp::resolver::query q("host", "service");
 * tcp::socket s(io_context);
 * tcp::endpoint e = egt::asio::connect(s,
...
template <typename Protocol EGT_ASIO_SVC_TPARAM,
    typename Iterator, typename ConnectCondition>
Iterator connect(basic_socket<Protocol EGT_ASIO_SVC_TARG>& s, Iterator begin,
    ConnectCondition connect_condition, egt::asio::error_code& ec,
    typename enable_if<!is_endpoint_sequence<Iterator>::value>::type* = 0);
#endif // !defined(EGT_ASIO_NO_DEPRECATED)
 ...
 * tcp::resolver::results_type e = r.resolve(q);
 * tcp::socket s(io_context);
 * tcp::resolver::results_type::iterator i = egt::asio::connect(
...
/**
 * @defgroup async_connect egt::asio::async_connect
 *
 ...
 *   // egt::asio::error::not_found. Otherwise, contains the
 ...
 *   if (!ec)
 *   {
 *     egt::asio::async_connect(s, results, connect_handler);
 *   }
 * }
 *
 * // ...
...
EGT_ASIO_INITFN_RESULT_TYPE(RangeConnectHandler,
...
EGT_ASIO_INITFN_RESULT_TYPE(IteratorConnectHandler,
 ...
 *   if (!ec)
 *   {
 *     egt::asio::async_connect(s, results,
...
EGT_ASIO_INITFN_RESULT_TYPE(IteratorConnectHandler,
    void (egt::asio::error_code, Iterator))
async_connect(basic_socket<Protocol EGT_ASIO_SVC_TARG>& s, Iterator begin,
 ...
 *     egt::asio::async_connect(s, i, end,
 *         my_connect_condition(),
 *         connect_handler);
 *   }
 * }
