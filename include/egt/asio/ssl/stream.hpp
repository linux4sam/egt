...
#ifndef EGT_ASIO_SSL_STREAM_HPP
#define EGT_ASIO_SSL_STREAM_HPP
...
#include <egt/asio/ssl/detail/buffered_handshake_op.hpp>
#include <egt/asio/ssl/detail/handshake_op.hpp>
#include <egt/asio/ssl/detail/io.hpp>
#include <egt/asio/ssl/detail/read_op.hpp>
#include <egt/asio/ssl/detail/shutdown_op.hpp>
#include <egt/asio/ssl/detail/stream_core.hpp>
#include <egt/asio/ssl/detail/write_op.hpp>
 ...
 * egt::asio::ssl::context ctx(egt::asio::ssl::context::sslv23);
 * egt::asio::ssl::stream<asio:ip::tcp::socket> sock(io_context, ctx);
 * @endcode
 *
 * @par Concepts:
...
public:
  ...
  stream(Arg&& arg, context& ctx)
    : next_layer_(EGT_ASIO_MOVE_CAST(Arg)(arg)),
      core_(ctx.native_handle(),
          next_layer_.lowest_layer().get_executor().context())
  {
  }
#else // defined(EGT_ASIO_HAS_MOVE) || defined(GENERATING_DOCUMENTATION)
  template <typename Arg>
  stream(Arg& arg, context& ctx)
    : next_layer_(arg),
      core_(ctx.native_handle(),
   ...
   * @note Calls @c SSL_set_verify.
   */
  EGT_ASIO_SYNC_OP_VOID set_verify_mode(
   ...
   * @note Calls @c SSL_set_verify_depth.
   */
  EGT_ASIO_SYNC_OP_VOID set_verify_depth(
  ...
  void handshake(handshake_type type)
  {
    egt::asio::error_code ec;
    handshake(type, ec);
    egt::asio::detail::throw_error(ec, "handshake");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  EGT_ASIO_SYNC_OP_VOID handshake(handshake_type type,
      egt::asio::error_code& ec)
  {
  ...
  EGT_ASIO_INITFN_RESULT_TYPE(HandshakeHandler,
      void (egt::asio::error_code))
  async_handshake(handshake_type type,
      EGT_ASIO_MOVE_ARG(HandshakeHandler) handler)
  {
    // If you get an error on the following line it means that your handler does
    // not meet the documented type requirements for a HandshakeHandler.
    EGT_ASIO_HANDSHAKE_HANDLER_CHECK(HandshakeHandler, handler) type_check;

    egt::asio::async_completion<HandshakeHandler,
  ...
  EGT_ASIO_INITFN_RESULT_TYPE(BufferedHandshakeHandler,
      void (egt::asio::error_code, std::size_t))
  async_handshake(handshake_type type, const ConstBufferSequence& buffers,
      EGT_ASIO_MOVE_ARG(BufferedHandshakeHandler) handler)
  {
    // If you get an error on the following line it means that your handler does
    // not meet the documented type requirements for a BufferedHandshakeHandler.
    EGT_ASIO_BUFFERED_HANDSHAKE_HANDLER_CHECK(
        BufferedHandshakeHandler, handler) type_check;

    egt::asio::async_completion<BufferedHandshakeHandler,
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  EGT_ASIO_SYNC_OP_VOID shutdown(egt::asio::error_code& ec)
  {
  ...
  EGT_ASIO_INITFN_RESULT_TYPE(ShutdownHandler,
      void (egt::asio::error_code))
  async_shutdown(EGT_ASIO_MOVE_ARG(ShutdownHandler) handler)
  {
    // If you get an error on the following line it means that your handler does
    // not meet the documented type requirements for a ShutdownHandler.
    EGT_ASIO_SHUTDOWN_HANDLER_CHECK(ShutdownHandler, handler) type_check;

    egt::asio::async_completion<ShutdownHandler,
      void (egt::asio::error_code)> init(handler);
