...
#ifndef EGT_ASIO_SSL_DETAIL_IMPL_ENGINE_IPP
#define EGT_ASIO_SSL_DETAIL_IMPL_ENGINE_IPP
...
#include <egt/asio/ssl/verify_context.hpp>
  ...
  : ssl_(::SSL_new(context))
{
  if (!ssl_)
  {
    ...
    egt::asio::detail::throw_error(ec, "engine");
  }
...
egt::asio::error_code engine::set_verify_mode(
...
egt::asio::error_code engine::set_verify_depth(
...
egt::asio::error_code engine::set_verify_callback(
...
{
  return perform((type == egt::asio::ssl::stream_base::client)
      ? &engine::do_connect : &engine::do_accept, 0, 0, ec, 0);
}

engine::want engine::shutdown(egt::asio::error_code& ec)
{
  return perform(&engine::do_shutdown, 0, 0, ec, 0);
}

engine::want engine::write(const egt::asio::const_buffer& data,
...
engine::want engine::read(const egt::asio::mutable_buffer& data,
    egt::asio::error_code& ec, std::size_t& bytes_transferred)
{
  if (data.size() == 0)
  {
...
egt::asio::mutable_buffer engine::get_output(
    const egt::asio::mutable_buffer& data)
{
  int length = ::BIO_read(ext_bio_,
      data.data(), static_cast<int>(data.size()));

  return egt::asio::buffer(data,
      length > 0 ? static_cast<std::size_t>(length) : 0);
}

egt::asio::const_buffer engine::put_input(
...
const egt::asio::error_code& engine::map_error_code(
    egt::asio::error_code& ec) const
{
  // We only want to map the error::eof code.
  if (ec != egt::asio::error::eof)
    return ec;

  // If there's data yet to be read, it's an error.
  if (BIO_wpending(ext_bio_))
  {
    ec = egt::asio::ssl::error::stream_truncated;
    return ec;
  }
...
#if (OPENSSL_VERSION_NUMBER < 0x10000000L)
egt::asio::detail::static_mutex& engine::accept_mutex()
{
  static egt::asio::detail::static_mutex mutex = EGT_ASIO_STATIC_MUTEX_INIT;
  return mutex;
}
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)
    ...
    std::size_t* bytes_transferred)
{
  ...
  {
    ec = egt::asio::error_code(sys_error,
        egt::asio::error::get_ssl_category());
    return want_nothing;
  }
...
{
#if (OPENSSL_VERSION_NUMBER < 0x10000000L)
  egt::asio::detail::static_mutex::scoped_lock lock(accept_mutex());
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)
...
#endif // EGT_ASIO_SSL_DETAIL_IMPL_ENGINE_IPP
