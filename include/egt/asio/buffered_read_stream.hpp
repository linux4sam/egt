...
#ifndef EGT_ASIO_BUFFERED_READ_STREAM_HPP
#define EGT_ASIO_BUFFERED_READ_STREAM_HPP
...
#include <egt/asio/detail/buffer_resize_guard.hpp>
#include <egt/asio/detail/buffered_stream_storage.hpp>
...
public:
...
#else
  EGT_ASIO_STATIC_CONSTANT(std::size_t, default_buffer_size = 1024);
#endif
  ...
  std::size_t fill(egt::asio::error_code& ec);

  /// Start an asynchronous fill.
  template <typename ReadHandler>
  EGT_ASIO_INITFN_RESULT_TYPE(ReadHandler,
      void (egt::asio::error_code, std::size_t))
  async_fill(EGT_ASIO_MOVE_ARG(ReadHandler) handler);
  ...
  /// Determine the amount of data that may be read without blocking.
  std::size_t in_avail(egt::asio::error_code& ec)
  {
...
private:
  ...
  {
    std::size_t bytes_copied = egt::asio::buffer_copy(
  ...
  {
    return egt::asio::buffer_copy(buffers, storage_.data(), storage_.size());
  }
...
#include <egt/asio/impl/buffered_read_stream.hpp>

#endif // EGT_ASIO_BUFFERED_READ_STREAM_HPP
