...
#ifndef EGT_ASIO_EXPERIMENTAL_IMPL_CO_SPAWN_HPP
#define EGT_ASIO_EXPERIMENTAL_IMPL_CO_SPAWN_HPP
...
{
public:
#if !defined(EGT_ASIO_DISABLE_AWAITEE_RECYCLING)
  void* operator new(std::size_t size)
  {
    return egt::asio::detail::thread_info_base::allocate(
        egt::asio::detail::thread_info_base::awaitee_tag(),
        egt::asio::detail::thread_context::thread_call_stack::top(),
        size);
  }

  void operator delete(void* pointer, std::size_t size)
  {
    egt::asio::detail::thread_info_base::deallocate(
        egt::asio::detail::thread_info_base::awaitee_tag(),
        egt::asio::detail::thread_context::thread_call_stack::top(),
        pointer, size);
  }
#endif // !defined(EGT_ASIO_DISABLE_AWAITEE_RECYCLING)
...
template <typename Executor>
class await_handler<Executor, egt::asio::error_code>
  : public await_handler_base<Executor, void>
{
public:
  ...
  void operator()(const egt::asio::error_code& ec)
  {
    ...
    if (ec)
    {
      this->awaitee_->set_except(
          std::make_exception_ptr(egt::asio::system_error(ec)));
    }
    else
      this->awaitee_->return_void();
    this->awaitee_->wake_caller();
...
template <typename Executor, typename T>
class await_handler<Executor, egt::asio::error_code, T>
  : public await_handler_base<Executor, T>
{
public:
  using await_handler_base<Executor, T>::await_handler_base;
  ...
  template <typename Arg>
  void operator()(const egt::asio::error_code& ec, Arg&& arg)
  {
    typename awaiter<Executor>::ptr ptr(std::move(this->awaiter_));
    if (ec)
    {
...
template <typename Executor, typename... Ts>
class await_handler<Executor, egt::asio::error_code, Ts...>
  : public await_handler_base<Executor, std::tuple<Ts...>>
{
public:
  using await_handler_base<Executor, std::tuple<Ts...>>::await_handler_base;
  ...
  template <typename... Args>
  void operator()(const egt::asio::error_code& ec, Args&&... args)
  {
    typename awaiter<Executor>::ptr ptr(std::move(this->awaiter_));
    if (ec)
    {
...
struct coroutine_traits<
  egt::asio::experimental::detail::awaiter<Executor>*, Args...>
{
  typedef egt::asio::experimental::detail::awaiter<Executor> promise_type;
};

template <typename T, typename Executor, typename... Args>
struct coroutine_traits<
  egt::asio::experimental::awaitable<T, Executor>, Args...>
{
  typedef egt::asio::experimental::detail::awaitee<T, Executor> promise_type;
};
...
#endif // EGT_ASIO_EXPERIMENTAL_IMPL_CO_SPAWN_HPP
