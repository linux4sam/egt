...
#ifndef EGT_ASIO_SPAWN_HPP
#define EGT_ASIO_SPAWN_HPP
...
public:
   ...
   * } @endcode
   */
  basic_yield_context operator[](egt::asio::error_code& ec) const
  {
 ...
 *
 * @code egt::asio::spawn(my_strand, do_echo);
 *
 * // ...
 *
 * void do_echo(egt::asio::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           egt::asio::buffer(data), yield);
 *
 *       egt::asio::async_write(my_socket,
 *           egt::asio::buffer(data, length), yield);
 *     }
 *   }
...
void spawn(EGT_ASIO_MOVE_ARG(Function) function,
 ...
 */
template <typename Function>
void spawn(const egt::asio::io_context::strand& s,
    EGT_ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes& attributes
      = boost::coroutines::attributes());
...
#include <egt/asio/impl/spawn.hpp>

#endif // EGT_ASIO_SPAWN_HPP
