...
#ifndef EGT_ASIO_WINDOWS_BASIC_STREAM_HANDLE_HPP
#define EGT_ASIO_WINDOWS_BASIC_STREAM_HANDLE_HPP
...
public:
   ...
   */
  explicit basic_stream_handle(egt::asio::io_context& io_context)
   ...
   */
  basic_stream_handle(egt::asio::io_context& io_context,
  ...
  basic_stream_handle(basic_stream_handle&& other)
    : basic_handle<StreamHandleService>(
        EGT_ASIO_MOVE_CAST(basic_stream_handle)(other))
  {
  }
   ...
   * @code
   * handle.write_some(egt::asio::buffer(data, size));
   * @endcode
   ...
   * To write a single data buffer use the @ref buffer function as follows:
   * @code
   * handle.async_write_some(egt::asio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   ...
   * @code
   * handle.read_some(egt::asio::buffer(data, size));
   * @endcode
   ...
   * To read into a single data buffer use the @ref buffer function as follows:
   * @code
   * handle.async_read_some(egt::asio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on reading into multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
...
#endif // EGT_ASIO_WINDOWS_BASIC_STREAM_HANDLE_HPP
