...
#ifndef EGT_ASIO_WINDOWS_BASIC_RANDOM_ACCESS_HANDLE_HPP
#define EGT_ASIO_WINDOWS_BASIC_RANDOM_ACCESS_HANDLE_HPP
...
public:
   ...
   * handle.
   */
  explicit basic_random_access_handle(egt::asio::io_context& io_context)
   ...
   */
  basic_random_access_handle(egt::asio::io_context& io_context,
  ...
  basic_random_access_handle(basic_random_access_handle&& other)
    : basic_handle<RandomAccessHandleService>(
        EGT_ASIO_MOVE_CAST(basic_random_access_handle)(other))
  {
  }
   ...
   * @code
   * handle.write_some_at(42, egt::asio::buffer(data, size));
   * @endcode
      ...
      const ConstBufferSequence& buffers)
  {
    ...
    egt::asio::detail::throw_error(ec, "write_some_at");
    return s;
  }
   ...
   * To write a single data buffer use the @ref buffer function as follows:
   * @code
   * handle.async_write_some_at(42, egt::asio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
      ...
      EGT_ASIO_MOVE_ARG(WriteHandler) handler)
  {
    ...
    return this->get_service().async_write_some_at(this->get_implementation(),
        offset, buffers, EGT_ASIO_MOVE_CAST(WriteHandler)(handler));
  }
   ...
   * @code
   * handle.read_some_at(42, egt::asio::buffer(data, size));
   * @endcode
      ...
      const MutableBufferSequence& buffers)
  {
    ...
    std::size_t s = this->get_service().read_some_at(
        this->get_implementation(), offset, buffers, ec);
    egt::asio::detail::throw_error(ec, "read_some_at");
    return s;
  }
   ...
   * To read into a single data buffer use the @ref buffer function as follows:
   * @code
   * handle.async_read_some_at(42, egt::asio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on reading into multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
      ...
      EGT_ASIO_MOVE_ARG(ReadHandler) handler)
  {
    ...
    return this->get_service().async_read_some_at(this->get_implementation(),
        offset, buffers, EGT_ASIO_MOVE_CAST(ReadHandler)(handler));
  }
};
...
#endif // EGT_ASIO_WINDOWS_BASIC_RANDOM_ACCESS_HANDLE_HPP
