...
#ifndef EGT_ASIO_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
#define EGT_ASIO_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
...
#include <egt/asio/detail/win_iocp_operation.hpp>
#include <egt/asio/detail/win_iocp_thread_info.hpp>
...
public:
  ...
  EGT_ASIO_DECL win_iocp_io_context(egt::asio::execution_context& ctx,
  ...
  EGT_ASIO_DECL egt::asio::error_code register_handle(
      HANDLE handle, egt::asio::error_code& ec);

  // Run the event loop until stopped or no more work.
  EGT_ASIO_DECL size_t run(egt::asio::error_code& ec);

  // Run until stopped or one operation is performed.
  EGT_ASIO_DECL size_t run_one(egt::asio::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  EGT_ASIO_DECL size_t wait_one(long usec, egt::asio::error_code& ec);

  // Poll for operations without blocking.
  EGT_ASIO_DECL size_t poll(egt::asio::error_code& ec);

  // Poll for one operation without blocking.
  EGT_ASIO_DECL size_t poll_one(egt::asio::error_code& ec);
  ...
  EGT_ASIO_DECL void post_deferred_completion(win_iocp_operation* op);

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() was previously called for the operations.
  EGT_ASIO_DECL void post_deferred_completions(
  ...
  EGT_ASIO_DECL void on_pending(win_iocp_operation* op);

  // Called after starting an overlapped I/O operation that completed
  // immediately. The caller must have already called work_started() prior to
  // starting the operation.
  EGT_ASIO_DECL void on_completion(win_iocp_operation* op,
  ...
  // starting the operation.
  EGT_ASIO_DECL void on_completion(win_iocp_operation* op,
      const egt::asio::error_code& ec, DWORD bytes_transferred = 0);
...
private:
  ...
  // either 0 or 1).
  EGT_ASIO_DECL size_t do_one(DWORD msec, egt::asio::error_code& ec);

  // Helper to calculate the GetQueuedCompletionStatus timeout.
  EGT_ASIO_DECL static DWORD get_gqcs_timeout();
  ...
  EGT_ASIO_DECL void update_timeout();
...
#include <egt/asio/detail/impl/win_iocp_io_context.hpp>
...
#endif // EGT_ASIO_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
