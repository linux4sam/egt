...
#ifndef EGT_ASIO_DETAIL_SERVICE_REGISTRY_HPP
#define EGT_ASIO_DETAIL_SERVICE_REGISTRY_HPP
...
public:
  // Constructor.
  EGT_ASIO_DECL service_registry(execution_context& owner);

  // Destructor.
  EGT_ASIO_DECL ~service_registry();

  // Shutdown all services.
  EGT_ASIO_DECL void shutdown_services();

  // Destroy all services.
  EGT_ASIO_DECL void destroy_services();

  // Notify all services of a fork event.
  EGT_ASIO_DECL void notify_fork(execution_context::fork_event fork_ev);
...
private:
  ...
  EGT_ASIO_DECL static void init_key_from_id(
  ...
  EGT_ASIO_DECL static bool keys_match(
  ...
  EGT_ASIO_DECL static void destroy(execution_context::service* service);
  ...
  // create a new service object automatically if no such object already
  // exists. Ownership of the service object is not transferred to the caller.
  EGT_ASIO_DECL execution_context::service* do_use_service(
      const execution_context::service::key& key,
      factory_type factory, void* owner);

  // Add a service object. Throws on error, in which case ownership of the
  // object is retained by the caller.
  EGT_ASIO_DECL void do_add_service(
      const execution_context::service::key& key,
      execution_context::service* new_service);

  // Check whether a service object with the specified key already exists.
  EGT_ASIO_DECL bool do_has_service(
      const execution_context::service::key& key) const;

  // Mutex to protect access to internal data.
  mutable egt::asio::detail::mutex mutex_;
