...
#ifndef EGT_ASIO_DETAIL_EPOLL_REACTOR_HPP
#define EGT_ASIO_DETAIL_EPOLL_REACTOR_HPP
...
#include <egt/asio/detail/object_pool.hpp>
...
public:
  ...
  class descriptor_state : operation
  {
    ...
    EGT_ASIO_DECL descriptor_state(bool locking);
    void set_ready_events(uint32_t events) { task_result_ = events; }
    void add_ready_events(uint32_t events) { task_result_ |= events; }
    EGT_ASIO_DECL operation* perform_io(uint32_t events);
    EGT_ASIO_DECL static void do_complete(
  ...
  // Constructor.
  EGT_ASIO_DECL epoll_reactor(egt::asio::execution_context& ctx);

  // Destructor.
  EGT_ASIO_DECL ~epoll_reactor();
  ...
  // code on failure.
  EGT_ASIO_DECL int register_descriptor(socket_type descriptor,
  ...
  EGT_ASIO_DECL void cancel_ops(socket_type descriptor,
      per_descriptor_data& descriptor_data);
  ...
  // descriptor data.
  EGT_ASIO_DECL void cleanup_descriptor_data(
      per_descriptor_data& descriptor_data);
...
private:
  ...
  // cannot be created.
  EGT_ASIO_DECL static int do_epoll_create();

  // Create the timerfd file descriptor. Does not throw.
  EGT_ASIO_DECL static int do_timerfd_create();

  // Allocate a new descriptor state object.
  EGT_ASIO_DECL descriptor_state* allocate_descriptor_state();

  // Free an existing descriptor state object.
  EGT_ASIO_DECL void free_descriptor_state(descriptor_state* s);
  ...
  EGT_ASIO_DECL int get_timeout(int msec);

#if defined(EGT_ASIO_HAS_TIMERFD)
  // Get the timeout value for the timer descriptor. The return value is the
  // flag argument to be used when calling timerfd_settime.
  EGT_ASIO_DECL int get_timeout(itimerspec& ts);
#endif // defined(EGT_ASIO_HAS_TIMERFD)
...
#include <egt/asio/detail/impl/epoll_reactor.hpp>
...
#endif // EGT_ASIO_DETAIL_EPOLL_REACTOR_HPP
