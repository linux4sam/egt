...
#ifndef EGT_ASIO_DETAIL_STRAND_SERVICE_HPP
#define EGT_ASIO_DETAIL_STRAND_SERVICE_HPP
...
class strand_service
  : public egt::asio::detail::service_base<strand_service>
{
private:
...
public:
  ...
  EGT_ASIO_DECL explicit strand_service(egt::asio::io_context& io_context);
  ...
  EGT_ASIO_DECL bool running_in_this_thread(
      const implementation_type& impl) const;

private:
  // Helper function to dispatch a handler. Returns true if the handler should
  // be dispatched immediately.
  EGT_ASIO_DECL bool do_dispatch(implementation_type& impl, operation* op);

  // Helper fiunction to post a handler.
  EGT_ASIO_DECL void do_post(implementation_type& impl,
      operation* op, bool is_continuation);

  EGT_ASIO_DECL static void do_complete(void* owner,
      operation* base, const egt::asio::error_code& ec,
...
#if defined(EGT_ASIO_STRAND_IMPLEMENTATIONS)
  enum { num_implementations = EGT_ASIO_STRAND_IMPLEMENTATIONS };
#else // defined(EGT_ASIO_STRAND_IMPLEMENTATIONS)
  enum { num_implementations = 193 };
#endif // defined(EGT_ASIO_STRAND_IMPLEMENTATIONS)
