...
#ifndef EGT_ASIO_DETAIL_CONSUMING_BUFFERS_HPP
#define EGT_ASIO_DETAIL_CONSUMING_BUFFERS_HPP
...
public:
  ...
  prepared_buffers_type prepare(std::size_t max_size)
  {
    ...
    Buffer_Iterator next = egt::asio::buffer_sequence_begin(buffers_);
    Buffer_Iterator end = egt::asio::buffer_sequence_end(buffers_);
    ...
    while (next != end && max_size > 0 && (result.count) < result.max_buffers)
    {
      ...
      result.elems[result.count] = egt::asio::buffer(next_buf, max_size);
...
public:
  ...
  {
    return egt::asio::buffer(buffer_ + total_consumed_, max_size);
  }
...
class consuming_buffers<mutable_buffer, mutable_buffer, const mutable_buffer*>
  : public consuming_single_buffer<EGT_ASIO_MUTABLE_BUFFER>
{
public:
  explicit consuming_buffers(const mutable_buffer& buffer)
    : consuming_single_buffer<EGT_ASIO_MUTABLE_BUFFER>(buffer)
  {
  }
};
...
class consuming_buffers<const_buffer, mutable_buffer, const mutable_buffer*>
  : public consuming_single_buffer<EGT_ASIO_CONST_BUFFER>
{
public:
  explicit consuming_buffers(const mutable_buffer& buffer)
    : consuming_single_buffer<EGT_ASIO_CONST_BUFFER>(buffer)
  {
  }
};
...
public:
  ...
  boost::array<Buffer, 2> prepare(std::size_t max_size)
  {
    ...
    result[0] = egt::asio::buffer(result[0] + total_consumed_, max_size);
    result[1] = egt::asio::buffer(
...
#endif // EGT_ASIO_DETAIL_CONSUMING_BUFFERS_HPP
