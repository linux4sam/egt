...
#ifndef EGT_ASIO_DETAIL_WIN_IOCP_SERIAL_PORT_SERVICE_HPP
#define EGT_ASIO_DETAIL_WIN_IOCP_SERIAL_PORT_SERVICE_HPP
...
#if defined(EGT_ASIO_HAS_IOCP) && defined(EGT_ASIO_HAS_SERIAL_PORT)
...
public:
  ...
  // Constructor.
  EGT_ASIO_DECL win_iocp_serial_port_service(
  ...
  EGT_ASIO_DECL egt::asio::error_code open(implementation_type& impl,
  ...
  // Destroy a serial port implementation.
  egt::asio::error_code close(implementation_type& impl,
  ...
  egt::asio::error_code cancel(implementation_type& impl,
      egt::asio::error_code& ec)
  {
  ...
  egt::asio::error_code send_break(implementation_type&,
      egt::asio::error_code& ec)
  {
...
private:
  ...
  typedef egt::asio::error_code (*store_function_type)(
      const void*, ::DCB&, egt::asio::error_code&);

  // Helper function template to store a serial port option.
  template <typename SettableSerialPortOption>
  static egt::asio::error_code store_option(const void* option,
      ::DCB& storage, egt::asio::error_code& ec)
  {
  ...
  EGT_ASIO_DECL egt::asio::error_code do_set_option(
      ...
      const void* option, egt::asio::error_code& ec);

  // Function pointer type for loading a serial port option.
  typedef egt::asio::error_code (*load_function_type)(
      void*, const ::DCB&, egt::asio::error_code&);

  // Helper function template to load a serial port option.
  template <typename GettableSerialPortOption>
  static egt::asio::error_code load_option(void* option,
      const ::DCB& storage, egt::asio::error_code& ec)
  {
  ...
  EGT_ASIO_DECL egt::asio::error_code do_get_option(
      const implementation_type& impl, load_function_type load,
      void* option, egt::asio::error_code& ec) const;
...
#endif // defined(EGT_ASIO_HAS_IOCP) && defined(EGT_ASIO_HAS_SERIAL_PORT)

#endif // EGT_ASIO_DETAIL_WIN_IOCP_SERIAL_PORT_SERVICE_HPP
