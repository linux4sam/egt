...
#ifndef EGT_ASIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
#define EGT_ASIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
...
dev_poll_reactor::dev_poll_reactor(egt::asio::execution_context& ctx)
  : egt::asio::detail::execution_context_service_base<dev_poll_reactor>(ctx),
...
{
  egt::asio::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, egt::asio::error::operation_aborted);
}
...
void dev_poll_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  if (events_size > 0)
  {
    ...
    {
      egt::asio::error_code ec = egt::asio::error_code(
...
int dev_poll_reactor::do_dev_poll_create()
{
  ...
  if (fd == -1)
  {
    egt::asio::error_code ec(errno,
        egt::asio::error::get_system_category());
    egt::asio::detail::throw_error(ec, "/dev/poll");
  }
  return fd;
}
...
#endif // EGT_ASIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
