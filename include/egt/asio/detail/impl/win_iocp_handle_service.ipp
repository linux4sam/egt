...
#ifndef EGT_ASIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
#define EGT_ASIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
...
{
public:
  explicit overlapped_wrapper(egt::asio::error_code& ec)
  {
    Internal = 0;
    InternalHigh = 0;
    Offset = 0;
...
win_iocp_handle_service::win_iocp_handle_service(
  ...
  : service_base<win_iocp_handle_service>(io_context),
    iocp_service_(egt::asio::use_service<win_iocp_io_context>(io_context)),
    mutex_(),
    impl_list_(0)
{
}
...
egt::asio::error_code win_iocp_handle_service::assign(
    win_iocp_handle_service::implementation_type& impl,
...
egt::asio::error_code win_iocp_handle_service::close(
    win_iocp_handle_service::implementation_type& impl,
    egt::asio::error_code& ec)
{
  if (is_open(impl))
  {
    EGT_ASIO_HANDLER_OPERATION((iocp_service_.context(), "handle",
...
egt::asio::error_code win_iocp_handle_service::cancel(
    win_iocp_handle_service::implementation_type& impl,
    egt::asio::error_code& ec)
{
  if (!is_open(impl))
...
size_t win_iocp_handle_service::do_write(
    ...
    const egt::asio::const_buffer& buffer, egt::asio::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = egt::asio::error::bad_descriptor;
...
void win_iocp_handle_service::start_write_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const egt::asio::const_buffer& buffer, operation* op)
{
...
size_t win_iocp_handle_service::do_read(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const egt::asio::mutable_buffer& buffer, egt::asio::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = egt::asio::error::bad_descriptor;
...
void win_iocp_handle_service::start_read_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const egt::asio::mutable_buffer& buffer, operation* op)
{
  update_cancellation_thread_id(impl);
  iocp_service_.work_started();
...
#endif // EGT_ASIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
