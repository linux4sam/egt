...
#ifndef EGT_ASIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
#define EGT_ASIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
...
#include <egt/asio/detail/win_iocp_socket_service_base.hpp>
    ...
    win_iocp_socket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    egt::asio::detail::mutex::scoped_lock lock(other_service.mutex_);
...
egt::asio::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type& impl,
    egt::asio::error_code& ec)
{
  if (is_open(impl))
  {
    EGT_ASIO_HANDLER_OPERATION((iocp_service_.context(),
...
egt::asio::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type& impl,
    egt::asio::error_code& ec)
{
  if (!is_open(impl))
        ...
        ::GetModuleHandleA("KERNEL32"), "CancelIoEx"))
  {
    ...
    if (!cancel_io_ex(sock_as_handle, 0))
    {
      ...
      {
        ec = egt::asio::error_code(last_error,
...
#else // defined(EGT_ASIO_ENABLE_CANCELIO)
  else
  {
...
egt::asio::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int family, int type, int protocol, egt::asio::error_code& ec)
{
  if (is_open(impl))
  {
    ec = egt::asio::error::already_open;
    return ec;
  }
...
egt::asio::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int type, socket_type native_socket, egt::asio::error_code& ec)
{
  if (is_open(impl))
  {
    ec = egt::asio::error::already_open;
    ...
    socket_base::message_flags flags, bool noop, operation* op)
{
  ...
  if (noop)
    ...
    iocp_service_.on_completion(op, egt::asio::error::bad_descriptor);
  else
  {
    ...
    int protocol, void* output_buffer, DWORD address_length, operation* op)
{
  ...
  if (!is_open(impl))
    ...
    iocp_service_.on_completion(op, egt::asio::error::already_open);
  else
  {
    egt::asio::error_code ec;
    ...
    std::size_t addrlen, win_iocp_socket_connect_op_base* op)
{
  ...
  if (family == EGT_ASIO_OS_DEF(AF_INET)
      || family == EGT_ASIO_OS_DEF(AF_INET6))
  {
    if (connect_ex_fn connect_ex = get_connect_ex(impl, type))
    {
      ...
      socket_ops::bind(impl.socket_, &a.base,
          family == EGT_ASIO_OS_DEF(AF_INET)
          ? sizeof(a.v4) : sizeof(a.v6), op->ec_);
      if (op->ec_ && op->ec_ != egt::asio::error::invalid_argument)
      {
        iocp_service_.post_immediate_completion(op, false);
        return;
      }
        ...
        impl.socket_, impl.state_, true, op->ec_))
  {
    ...
    {
      if (op->ec_ == egt::asio::error::in_progress
          || op->ec_ == egt::asio::error::would_block)
      {
...
{
#if defined(EGT_ASIO_DISABLE_CONNECTEX)
  (void)impl;
  (void)type;
  return 0;
#else // defined(EGT_ASIO_DISABLE_CONNECTEX)
  if (type != EGT_ASIO_OS_DEF(SOCK_STREAM)
      && type != EGT_ASIO_OS_DEF(SOCK_SEQPACKET))
    return 0;
...
#endif // defined(EGT_ASIO_DISABLE_CONNECTEX)
}
...
#endif // EGT_ASIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
