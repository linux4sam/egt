...
#ifndef EGT_ASIO_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
#define EGT_ASIO_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
...
win_iocp_io_context::win_iocp_io_context(
    egt::asio::execution_context& ctx, int concurrency_hint)
    ...
    concurrency_hint_(concurrency_hint)
{
  ...
  if (!iocp_.handle)
  {
    ...
    egt::asio::detail::throw_error(ec, "iocp");
  }
}
...
egt::asio::error_code win_iocp_io_context::register_handle(
...
size_t win_iocp_io_context::run(egt::asio::error_code& ec)
{
...
size_t win_iocp_io_context::run_one(egt::asio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::wait_one(long usec, egt::asio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll(egt::asio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll_one(egt::asio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
  ...
  if (::InterlockedExchange(&stopped_, 1) == 0)
  {
    if (::InterlockedExchange(&stop_event_posted_, 1) == 0)
    {
      if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0))
      {
        ...
        egt::asio::error_code ec(last_error,
            egt::asio::error::get_system_category());
        egt::asio::detail::throw_error(ec, "pqcs");
      }
    }
  }
}
...
void win_iocp_io_context::on_completion(win_iocp_operation* op,
    const egt::asio::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
...
size_t win_iocp_io_context::do_one(DWORD msec, egt::asio::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      egt::asio::error_code result_ec(last_error,
          egt::asio::error::get_system_category());
      ...
      {
        result_ec = egt::asio::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<egt::asio::error_category*>(op->Internal));
...
void win_iocp_io_context::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!waitable_timer_.handle)
  {
    ...
    if (waitable_timer_.handle == 0)
    {
      ...
      egt::asio::error_code ec(last_error,
          egt::asio::error::get_system_category());
      egt::asio::detail::throw_error(ec, "timer");
    }
...
#endif // EGT_ASIO_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
