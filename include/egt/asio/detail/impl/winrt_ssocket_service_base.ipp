...
#ifndef EGT_ASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
#define EGT_ASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
...
#include <cstring>
#include <egt/asio/detail/winrt_ssocket_service_base.hpp>
#include <egt/asio/detail/winrt_async_op.hpp>
#include <egt/asio/detail/winrt_utils.hpp>
...
egt::asio::error_code winrt_ssocket_service_base::close(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    void* addr, std::size_t addr_len, egt::asio::error_code& ec) const
{
  ...
  try
  {
    ...
    case EGT_ASIO_OS_DEF(AF_INET):
      ...
      {
        socket_ops::inet_pton(EGT_ASIO_OS_DEF(AF_INET), addr_string.c_str(),
    ...
    case EGT_ASIO_OS_DEF(AF_INET6):
      ...
      {
        socket_ops::inet_pton(EGT_ASIO_OS_DEF(AF_INET6), addr_string.c_str(),
...
egt::asio::error_code winrt_ssocket_service_base::do_set_option(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    std::size_t optlen, egt::asio::error_code& ec)
{
  ...
  try
  {
    if (level == EGT_ASIO_OS_DEF(SOL_SOCKET)
        && optname == EGT_ASIO_OS_DEF(SO_KEEPALIVE))
    {
    ...
    else if (level == EGT_ASIO_OS_DEF(IPPROTO_TCP)
        && optname == EGT_ASIO_OS_DEF(TCP_NODELAY))
    {
      if (optlen == sizeof(int))
      {
        int value = 0;
...
egt::asio::error_code winrt_ssocket_service_base::do_connect(
    winrt_ssocket_service_base::base_implementation_type& impl,
    const void* addr, egt::asio::error_code& ec)
{
  ...
  {
  case EGT_ASIO_OS_DEF(AF_INET):
    socket_ops::inet_ntop(EGT_ASIO_OS_DEF(AF_INET),
    ...
    break;
  case EGT_ASIO_OS_DEF(AF_INET6):
    socket_ops::inet_ntop(EGT_ASIO_OS_DEF(AF_INET6),
    ...
    const void* addr, winrt_async_op<void>* op, bool is_continuation)
{
  ...
  case EGT_ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    op->ec_ = egt::asio::error::address_family_not_supported;
    break;
  }
  ...
  catch (Platform::Exception^ e)
  {
    op->ec_ = egt::asio::error_code(
        e->HResult, egt::asio::system_category());
    io_context_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, egt::asio::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<egt::asio::const_buffer,
      egt::asio::const_buffer> bufs(egt::asio::buffer(data));
      ...
      winrt_async_op<unsigned int>* op, bool is_continuation)
{
  ...
  {
    op->ec_ = egt::asio::error_code(e->HResult,
        egt::asio::system_category());
    io_context_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, egt::asio::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<egt::asio::mutable_buffer,
        egt::asio::mutable_buffer> bufs(egt::asio::buffer(data));
...
#endif // EGT_ASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
