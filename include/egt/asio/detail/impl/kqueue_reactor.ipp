...
#ifndef EGT_ASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
#define EGT_ASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
...
# define EGT_ASIO_KQUEUE_EV_SET(ev, ident, filt, flags, fflags, data, udata) \
...
kqueue_reactor::kqueue_reactor(egt::asio::execution_context& ctx)
  : execution_context_service_base<kqueue_reactor>(ctx),
    scheduler_(use_service<scheduler>(ctx)),
    mutex_(EGT_ASIO_CONCURRENCY_HINT_IS_LOCKING(
    ...
    registered_descriptors_mutex_(mutex_.enabled())
{
  ...
  EGT_ASIO_KQUEUE_EV_SET(&events[0], interrupter_.read_descriptor(),
      EVFILT_READ, EV_ADD, 0, 0, &interrupter_);
  if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
  {
    egt::asio::error_code error(errno,
        egt::asio::error::get_system_category());
    egt::asio::detail::throw_error(error);
  }
}
...
{
  if (fork_ev == egt::asio::execution_context::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      egt::asio::error_code ec(errno,
          egt::asio::error::get_system_category());
      egt::asio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      ...
      {
        EGT_ASIO_KQUEUE_EV_SET(&events[0], state->descriptor_,
            EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, state);
        EGT_ASIO_KQUEUE_EV_SET(&events[1], state->descriptor_,
            EVFILT_WRITE, EV_ADD | EV_CLEAR, 0, 0, state);
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          egt::asio::error_code ec(errno,
              egt::asio::error::get_system_category());
          egt::asio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data)
{
  ...
  EGT_ASIO_HANDLER_REACTOR_REGISTRATION((
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, reactor_op* op)
{
  ...
  struct kevent events[1];
  EGT_ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
    ...
    bool is_continuation, bool allow_speculative)
{
  if (!descriptor_data)
  {
    op->ec_ = egt::asio::error::bad_descriptor;
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        EGT_ASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        EGT_ASIO_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        ...
        {
          op->ec_ = egt::asio::error_code(errno,
              egt::asio::error::get_system_category());
          scheduler_.post_immediate_completion(op, is_continuation);
          return;
        }
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, bool closing)
{
  ...
  if (!descriptor_data->shutdown_)
  {
    ...
    {
      struct kevent events[2];
      EGT_ASIO_KQUEUE_EV_SET(&events[0], descriptor,
          EVFILT_READ, EV_DELETE, 0, 0, 0);
      EGT_ASIO_KQUEUE_EV_SET(&events[1], descriptor,
    ...
    EGT_ASIO_HANDLER_REACTOR_DEREGISTRATION((
          context(), static_cast<uintmax_t>(descriptor),
          reinterpret_cast<uintmax_t>(descriptor_data)));
...
void kqueue_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    if (ptr != &interrupter_)
    {
      ...
      {
      case EVFILT_READ:
        event_mask |= EGT_ASIO_HANDLER_REACTOR_READ_EVENT;
        break;
      case EVFILT_WRITE:
        event_mask |= EGT_ASIO_HANDLER_REACTOR_WRITE_EVENT;
        break;
      }
      if ((events[i].flags & (EV_ERROR | EV_OOBAND)) != 0)
        event_mask |= EGT_ASIO_HANDLER_REACTOR_ERROR_EVENT;
      EGT_ASIO_HANDLER_REACTOR_EVENTS((context(),
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    else
    {
          ...
          && descriptor_data->op_queue_[write_op].empty())
      {
        ...
        EGT_ASIO_KQUEUE_EV_SET(&delete_events[0],
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    egt::asio::error_code ec(errno,
        egt::asio::error::get_system_category());
    egt::asio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
...
{
  mutex::scoped_lock descriptors_lock(registered_descriptors_mutex_);
  return registered_descriptors_.alloc(EGT_ASIO_CONCURRENCY_HINT_IS_LOCKING(
...
#undef EGT_ASIO_KQUEUE_EV_SET

#include <egt/asio/detail/pop_options.hpp>

#endif // defined(EGT_ASIO_HAS_KQUEUE)

#endif // EGT_ASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
