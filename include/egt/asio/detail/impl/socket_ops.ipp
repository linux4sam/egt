...
#ifndef EGT_ASIO_DETAIL_SOCKET_OPS_IPP
#define EGT_ASIO_DETAIL_SOCKET_OPS_IPP
...
{
#if defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
  ec = egt::asio::error_code(WSAGetLastError(),
...
socket_type accept(socket_type s, socket_addr_type* addr,
    std::size_t* addrlen, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
...
socket_type sync_accept(socket_type s, state_type state,
    socket_addr_type* addr, std::size_t* addrlen, egt::asio::error_code& ec)
{
  // Accept a socket.
  for (;;)
  {
    ...
    else if (ec == egt::asio::error::connection_aborted)
    {
...
void complete_iocp_accept(socket_type s,
    ...
    socket_type new_socket, egt::asio::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = egt::asio::error::connection_aborted;
...
bool non_blocking_accept(socket_type s,
    ...
    egt::asio::error_code& ec, socket_type& new_socket)
{
  for (;;)
  {
...
int close(socket_type s, state_type& state,
    bool destruction, egt::asio::error_code& ec)
{
  int result = 0;
  if (s != invalid_socket)
  {
...
int shutdown(socket_type s, int what, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
    ...
    std::size_t addrlen, egt::asio::error_code& ec)
{
  ...
  else if (ec == egt::asio::error::try_again)
    ec = egt::asio::error::no_buffer_space;
    ...
    std::size_t addrlen, egt::asio::error_code& ec)
{
  ...
  if (ec != egt::asio::error::in_progress
      && ec != egt::asio::error::would_block)
  {
  ...
  ec = egt::asio::error_code(connect_error,
      egt::asio::error::get_system_category());
}

#if defined(EGT_ASIO_HAS_IOCP)

void complete_iocp_connect(socket_type s, egt::asio::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  switch (ec.value())
  {
  ...
  case ERROR_NETWORK_UNREACHABLE:
    ec = egt::asio::error::network_unreachable;
    break;
  case ERROR_HOST_UNREACHABLE:
    ec = egt::asio::error::host_unreachable;
    break;
  case ERROR_SEM_TIMEOUT:
    ec = egt::asio::error::timed_out;
    break;
  default:
    break;
  }
...
bool non_blocking_connect(socket_type s, egt::asio::error_code& ec)
{
...
int socketpair(int af, int type, int protocol,
    socket_type sv[2], egt::asio::error_code& ec)
{
#if defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(af);
  (void)(type);
...
bool sockatmark(socket_type s, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
...
size_t available(socket_type s, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
...
int listen(socket_type s, int backlog, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
...
signed_size_type recv(socket_type s, buf* bufs, size_t count,
    int flags, egt::asio::error_code& ec)
{
  clear_last_error();
#if defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
...
size_t sync_recv(socket_type s, state_type state, buf* bufs,
    size_t count, int flags, bool all_empty, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
...
void complete_iocp_recv(state_type state,
    ...
    egt::asio::error_code& ec, size_t bytes_transferred)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
  {
...
signed_size_type recvmsg(socket_type s, buf* bufs, size_t count,
    int in_flags, int& out_flags, egt::asio::error_code& ec)
{
  clear_last_error();
#if defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
  out_flags = 0;
    ...
    egt::asio::error_code& ec)
{
  ...
  if (af == EGT_ASIO_OS_DEF(AF_INET6))
  {
...
int setsockopt(socket_type s, state_type& state, int level, int optname,
    const void* optval, std::size_t optlen, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
...
int getsockopt(socket_type s, state_type state, int level, int optname,
    void* optval, size_t* optlen, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
...
int getpeername(socket_type s, socket_addr_type* addr,
    std::size_t* addrlen, bool cached, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
  ...
  if (cached)
  {
    ...
    {
      ec = egt::asio::error::not_connected;
      return socket_error_retval;
    }
...
#else // defined(EGT_ASIO_WINDOWS) && !defined(EGT_ASIO_WINDOWS_APP)
      // || defined(__CYGWIN__)
  (void)cached;
...
int select(int nfds, fd_set* readfds, fd_set* writefds,
    fd_set* exceptfds, timeval* timeout, egt::asio::error_code& ec)
{
  clear_last_error();
#if defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
...
int poll_read(socket_type s, state_type state,
    int msec, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
  ...
  if (result == 0)
    ec = (state & user_set_non_blocking)
      ? egt::asio::error::would_block : egt::asio::error_code();
  else if (result > 0)
    ec = egt::asio::error_code();
  return result;
}
...
int poll_connect(socket_type s, int msec, egt::asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = egt::asio::error::bad_descriptor;
...
const char* inet_ntop(int af, const void* src, char* dest, size_t length,
    unsigned long scope_id, egt::asio::error_code& ec)
{
  clear_last_error();
#if defined(EGT_ASIO_WINDOWS_RUNTIME)
  using namespace std; // For sprintf.
  const unsigned char* bytes = static_cast<const unsigned char*>(src);
  if (af == EGT_ASIO_OS_DEF(AF_INET))
  {
    sprintf_s(dest, length, "%u.%u.%u.%u",
        bytes[0], bytes[1], bytes[2], bytes[3]);
    return dest;
  }
  else if (af == EGT_ASIO_OS_DEF(AF_INET6))
  {
    size_t n = 0, b = 0, z = 0;
    while (n < length && b < 16)
    {
  ...
  {
    ec = egt::asio::error::address_family_not_supported;
    return 0;
  }
#elif defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
  using namespace std; // For memcpy.

  if (af != EGT_ASIO_OS_DEF(AF_INET) && af != EGT_ASIO_OS_DEF(AF_INET6))
  {
    ec = egt::asio::error::address_family_not_supported;
    return 0;
  }
  ...
  if (af == EGT_ASIO_OS_DEF(AF_INET))
  {
    ...
    address.v4.sin_family = EGT_ASIO_OS_DEF(AF_INET);
  ...
  else // AF_INET6
  {
    ...
    address.v6.sin6_family = EGT_ASIO_OS_DEF(AF_INET6);
  ...
  if (result == 0 && !ec)
    ec = egt::asio::error::invalid_argument;
  if (result != 0 && af == EGT_ASIO_OS_DEF(AF_INET6) && scope_id != 0)
  {
    ...
    unsigned long* scope_id, egt::asio::error_code& ec)
{
  ...
  const bool is_v6 = (af == EGT_ASIO_OS_DEF(AF_INET6));
...
int gethostname(char* name, int namelen, egt::asio::error_code& ec)
{
  clear_last_error();
#if defined(EGT_ASIO_WINDOWS_RUNTIME)
  try
  ...
  {
    ec = egt::asio::error_code(e->HResult,
        egt::asio::system_category());
    return -1;
  }
#else // defined(EGT_ASIO_WINDOWS_RUNTIME)
...
inline egt::asio::error_code translate_netdb_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case HOST_NOT_FOUND:
    return egt::asio::error::host_not_found;
  case TRY_AGAIN:
    return egt::asio::error::host_not_found_try_again;
  case NO_RECOVERY:
    return egt::asio::error::no_recovery;
  case NO_DATA:
    return egt::asio::error::no_data;
  default:
    EGT_ASIO_ASSERT(false);
    return egt::asio::error::invalid_argument;
  }
}

inline hostent* gethostbyaddr(const char* addr, int length, int af,
    hostent* result, char* buffer, int buflength, egt::asio::error_code& ec)
{
  clear_last_error();
#if defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
...
inline hostent* gethostbyname(const char* name, int af, struct hostent* result,
    char* buffer, int buflength, int ai_flags, egt::asio::error_code& ec)
{
  clear_last_error();
#if defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
    ...
    const addrinfo_type* hints, gai_search (&search)[2])
{
  ...
  if (host == 0 || host[0] == '\0')
  {
    if (hints->ai_flags & AI_PASSIVE)
    {
      ...
      {
      case EGT_ASIO_OS_DEF(AF_INET):
        search[search_count].host = "0.0.0.0";
        search[search_count].family = EGT_ASIO_OS_DEF(AF_INET);
        ++search_count;
        break;
      case EGT_ASIO_OS_DEF(AF_INET6):
        search[search_count].host = "0::0";
        search[search_count].family = EGT_ASIO_OS_DEF(AF_INET6);
        ++search_count;
        break;
      case EGT_ASIO_OS_DEF(AF_UNSPEC):
        search[search_count].host = "0::0";
        search[search_count].family = EGT_ASIO_OS_DEF(AF_INET6);
        ++search_count;
        search[search_count].host = "0.0.0.0";
...
#else // defined(EGT_ASIO_HAS_SECURE_RTL)
  *target = 0;
  if (max_size > 0)
  ...
  case EGT_ASIO_OS_DEF(AF_INET):
    {
      ...
      if (sinptr == 0)
        return EAI_MEMORY;
      sinptr->sin_family = EGT_ASIO_OS_DEF(AF_INET);
  ...
  case EGT_ASIO_OS_DEF(AF_INET6):
    {
      ...
      if (sin6ptr == 0)
        return EAI_MEMORY;
      sin6ptr->sin6_family = EGT_ASIO_OS_DEF(AF_INET6);
    ...
    const addrinfo_type* hintsp, addrinfo_type** result)
{
  ...
  hints.ai_family = EGT_ASIO_OS_DEF(AF_UNSPEC);
  if (hintsp)
    hints = *hintsp;
  ...
  if (hints.ai_family != EGT_ASIO_OS_DEF(AF_INET6))
  ...
  for (gai_search* sptr = search; sptr < search + search_count; ++sptr)
  {
    ...
    egt::asio::error_code ec;
    if (socket_ops::inet_pton(EGT_ASIO_OS_DEF(AF_INET),
          sptr->host, &inaddr, 0, ec) == 1)
    {
      if (hints.ai_family != EGT_ASIO_OS_DEF(AF_UNSPEC)
          && hints.ai_family != EGT_ASIO_OS_DEF(AF_INET))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == EGT_ASIO_OS_DEF(AF_INET))
      {
        rc = gai_aistruct(&ainext, &hints, &inaddr, EGT_ASIO_OS_DEF(AF_INET));
        if (rc != 0)
        {
          freeaddrinfo_emulation(aihead);
          gai_free(canon);
    ...
    if (socket_ops::inet_pton(EGT_ASIO_OS_DEF(AF_INET6),
          sptr->host, &in6addr, 0, ec) == 1)
    {
      if (hints.ai_family != EGT_ASIO_OS_DEF(AF_UNSPEC)
          && hints.ai_family != EGT_ASIO_OS_DEF(AF_INET6))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == EGT_ASIO_OS_DEF(AF_INET6))
      {
    ...
    if (hptr == 0)
    {
      ...
      freeaddrinfo_emulation(aihead);
      gai_free(canon);
      if (ec == egt::asio::error::host_not_found)
        return EAI_NONAME;
      if (ec == egt::asio::error::host_not_found_try_again)
        return EAI_AGAIN;
      if (ec == egt::asio::error::no_recovery)
        return EAI_FAIL;
      if (ec == egt::asio::error::no_data)
        return EAI_NONAME;
      return EAI_NONAME;
    }
...
inline egt::asio::error_code getnameinfo_emulation(
    ...
    egt::asio::error_code& ec)
{
  ...
  case EGT_ASIO_OS_DEF(AF_INET):
    ...
    {
      return ec = egt::asio::error::invalid_argument;
    }
  ...
  case EGT_ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    return ec = egt::asio::error::address_family_not_supported;
  }
  ...
  if (host && hostlen > 0)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          return ec = egt::asio::error::host_not_found;
        }
  ...
  if (serv && servlen > 0)
  {
    if (flags & NI_NUMERICSERV)
    {
      if (servlen < 6)
      {
        return ec = egt::asio::error::no_buffer_space;
      }
#if defined(EGT_ASIO_HAS_SECURE_RTL)
...
inline egt::asio::error_code translate_addrinfo_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case EAI_FAIL:
    return egt::asio::error::no_recovery;
  case EAI_FAMILY:
    return egt::asio::error::address_family_not_supported;
  case EAI_MEMORY:
    return egt::asio::error::no_memory;
  case EAI_NONAME:
  ...
  case EAI_NODATA:
    ...
    return egt::asio::error::host_not_found;
  case EAI_SERVICE:
    return egt::asio::error::service_not_found;
  case EAI_SOCKTYPE:
    return egt::asio::error::socket_type_not_supported;
  default: // Possibly the non-portable EAI_SYSTEM.
#if defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
    return egt::asio::error_code(
        WSAGetLastError(), egt::asio::error::get_system_category());
#else
    return egt::asio::error_code(
        errno, egt::asio::error::get_system_category());
#endif
  }
}

egt::asio::error_code getaddrinfo(const char* host,
    const char* service, const addrinfo_type& hints,
    addrinfo_type** result, egt::asio::error_code& ec)
{
...
# else
  ...
  return ec = translate_addrinfo_error(error);
# endif
#elif !defined(EGT_ASIO_HAS_GETADDRINFO)
  int error = getaddrinfo_emulation(host, service, &hints, result);
  return ec = translate_addrinfo_error(error);
#else
  int error = ::getaddrinfo(host, service, &hints, result);
...
egt::asio::error_code background_getaddrinfo(
...
egt::asio::error_code getnameinfo(const socket_addr_type* addr,
    std::size_t addrlen, char* host, std::size_t hostlen,
    char* serv, std::size_t servlen, int flags, egt::asio::error_code& ec)
{
#if defined(EGT_ASIO_WINDOWS) || defined(__CYGWIN__)
# if defined(EGT_ASIO_HAS_GETADDRINFO)
  // Building for Windows XP, Windows Server 2003, or later.
...
egt::asio::error_code sync_getnameinfo(
    const socket_addr_type* addr, std::size_t addrlen,
    char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, egt::asio::error_code& ec)
{
...
egt::asio::error_code background_getnameinfo(
    const weak_cancel_token_type& cancel_token,
    const socket_addr_type* addr, std::size_t addrlen,
    char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, egt::asio::error_code& ec)
...
#endif // EGT_ASIO_DETAIL_SOCKET_OPS_IPP
