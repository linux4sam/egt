...
#ifndef EGT_ASIO_DETAIL_SCHEDULER_HPP
#define EGT_ASIO_DETAIL_SCHEDULER_HPP
...
#include <egt/asio/detail/conditionally_enabled_event.hpp>
#include <egt/asio/detail/conditionally_enabled_mutex.hpp>
...
public:
  ...
  EGT_ASIO_DECL scheduler(egt::asio::execution_context& ctx,
  ...
  EGT_ASIO_DECL std::size_t run(egt::asio::error_code& ec);

  // Run until interrupted or one operation is performed.
  EGT_ASIO_DECL std::size_t run_one(egt::asio::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  EGT_ASIO_DECL std::size_t wait_one(
      long usec, egt::asio::error_code& ec);

  // Poll for operations without blocking.
  EGT_ASIO_DECL std::size_t poll(egt::asio::error_code& ec);

  // Poll for one operation without blocking.
  EGT_ASIO_DECL std::size_t poll_one(egt::asio::error_code& ec);
  ...
  EGT_ASIO_DECL void compensating_work_started();
  ...
  EGT_ASIO_DECL void post_immediate_completion(
      operation* op, bool is_continuation);

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() was previously called for the operation.
  EGT_ASIO_DECL void post_deferred_completion(operation* op);

  // Request invocation of the given operations and return immediately. Assumes
  // that work_started() was previously called for each operation.
  EGT_ASIO_DECL void post_deferred_completions(op_queue<operation>& ops);

  // Enqueue the given operation following a failed attempt to dispatch the
  // operation for immediate invocation.
  EGT_ASIO_DECL void do_dispatch(operation* op);
  ...
  EGT_ASIO_DECL void abandon_operations(op_queue<operation>& ops);
...
private:
  ...
  EGT_ASIO_DECL std::size_t do_run_one(mutex::scoped_lock& lock,
      thread_info& this_thread, const egt::asio::error_code& ec);

  // Run at most one operation with a timeout. May block.
  EGT_ASIO_DECL std::size_t do_wait_one(mutex::scoped_lock& lock,
      thread_info& this_thread, long usec, const egt::asio::error_code& ec);

  // Poll for at most one operation.
  EGT_ASIO_DECL std::size_t do_poll_one(mutex::scoped_lock& lock,
      thread_info& this_thread, const egt::asio::error_code& ec);

  // Stop the task and all idle threads.
  EGT_ASIO_DECL void stop_all_threads(mutex::scoped_lock& lock);

  // Wake a single idle thread, or the task, and always unlock the mutex.
  EGT_ASIO_DECL void wake_one_thread_and_unlock(
...
#endif // EGT_ASIO_DETAIL_SCHEDULER_HPP
