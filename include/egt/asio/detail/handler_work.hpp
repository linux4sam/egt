...
#ifndef EGT_ASIO_DETAIL_HANDLER_WORK_HPP
#define EGT_ASIO_DETAIL_HANDLER_WORK_HPP
...
{
public:
  explicit handler_work(Handler& handler) EGT_ASIO_NOEXCEPT
    : executor_(associated_executor<Handler>::get(handler))
  {
  }

  static void start(Handler& handler) EGT_ASIO_NOEXCEPT
  {
  ...
  {
    executor_.dispatch(EGT_ASIO_MOVE_CAST(Function)(function),
...
{
public:
  explicit handler_work(Handler&) EGT_ASIO_NOEXCEPT {}
  static void start(Handler&) EGT_ASIO_NOEXCEPT {}
  ~handler_work() {}

  template <typename Function>
  void complete(Function& function, Handler& handler)
  {
    egt_asio_handler_invoke_helpers::invoke(function, handler);
  }
...
#endif // EGT_ASIO_DETAIL_HANDLER_WORK_HPP
