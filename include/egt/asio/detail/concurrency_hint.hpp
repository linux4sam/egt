...
#ifndef EGT_ASIO_DETAIL_CONCURRENCY_HINT_HPP
#define EGT_ASIO_DETAIL_CONCURRENCY_HINT_HPP
...
#define EGT_ASIO_CONCURRENCY_HINT_ID 0xA5100000u
#define EGT_ASIO_CONCURRENCY_HINT_ID_MASK 0xFFFF0000u

// If set, this bit indicates that the scheduler should perform locking.
#define EGT_ASIO_CONCURRENCY_HINT_LOCKING_SCHEDULER 0x1u

// If set, this bit indicates that the reactor should perform locking when
// managing descriptor registrations.
#define EGT_ASIO_CONCURRENCY_HINT_LOCKING_REACTOR_REGISTRATION 0x2u

// If set, this bit indicates that the reactor should perform locking for I/O.
#define EGT_ASIO_CONCURRENCY_HINT_LOCKING_REACTOR_IO 0x4u

// Helper macro to determine if we have a special concurrency hint.
#define EGT_ASIO_CONCURRENCY_HINT_IS_SPECIAL(hint) \
  ((static_cast<unsigned>(hint) \
    & EGT_ASIO_CONCURRENCY_HINT_ID_MASK) \
      == EGT_ASIO_CONCURRENCY_HINT_ID)

// Helper macro to determine if locking is enabled for a given facility.
#define EGT_ASIO_CONCURRENCY_HINT_IS_LOCKING(facility, hint) \
  (((static_cast<unsigned>(hint) \
    & (EGT_ASIO_CONCURRENCY_HINT_ID_MASK \
      | EGT_ASIO_CONCURRENCY_HINT_LOCKING_ ## facility)) \
        ^ EGT_ASIO_CONCURRENCY_HINT_ID) != 0)
...
#define EGT_ASIO_CONCURRENCY_HINT_UNSAFE \
  static_cast<int>(EGT_ASIO_CONCURRENCY_HINT_ID)
...
#define EGT_ASIO_CONCURRENCY_HINT_UNSAFE_IO \
  static_cast<int>(EGT_ASIO_CONCURRENCY_HINT_ID \
      | EGT_ASIO_CONCURRENCY_HINT_LOCKING_SCHEDULER \
      | EGT_ASIO_CONCURRENCY_HINT_LOCKING_REACTOR_REGISTRATION)

// The special concurrency hint provides full thread safety.
#define EGT_ASIO_CONCURRENCY_HINT_SAFE \
  static_cast<int>(EGT_ASIO_CONCURRENCY_HINT_ID \
      | EGT_ASIO_CONCURRENCY_HINT_LOCKING_SCHEDULER \
      | EGT_ASIO_CONCURRENCY_HINT_LOCKING_REACTOR_REGISTRATION \
      | EGT_ASIO_CONCURRENCY_HINT_LOCKING_REACTOR_IO)
...
#if !defined(EGT_ASIO_CONCURRENCY_HINT_DEFAULT)
# define EGT_ASIO_CONCURRENCY_HINT_DEFAULT -1
#endif // !defined(EGT_ASIO_CONCURRENCY_HINT_DEFAULT)

// This #define may be overridden at compile time to specify a program-wide
// concurrency hint, used by the one-argument io_context constructor when
// passed a value of 1.
#if !defined(EGT_ASIO_CONCURRENCY_HINT_1)
# define EGT_ASIO_CONCURRENCY_HINT_1 1
#endif // !defined(EGT_ASIO_CONCURRENCY_HINT_DEFAULT)

#endif // EGT_ASIO_DETAIL_CONCURRENCY_HINT_HPP
