...
#ifndef EGT_ASIO_BASIC_RAW_SOCKET_HPP
#define EGT_ASIO_BASIC_RAW_SOCKET_HPP
...
template <typename Protocol
    EGT_ASIO_SVC_TPARAM_DEF1(= raw_socket_service<Protocol>)>
...
public:
   ...
   * socket.
   */
  explicit basic_raw_socket(egt::asio::io_context& io_context)
   ...
   */
  basic_raw_socket(egt::asio::io_context& io_context,
  ...
  basic_raw_socket(basic_raw_socket<Protocol1 EGT_ASIO_SVC_TARG1>&& other,
  ...
  typename enable_if<is_convertible<Protocol1, Protocol>::value,
      basic_raw_socket>::type& operator=(
        basic_raw_socket<Protocol1 EGT_ASIO_SVC_TARG1>&& other)
  {
    basic_socket<Protocol EGT_ASIO_SVC_TARG>::operator=(std::move(other));
    return *this;
  }
      ...
      EGT_ASIO_MOVE_ARG(WriteHandler) handler)
  {
    ...
    return this->get_service().async_send_to(this->get_implementation(),
        buffers, destination, 0, EGT_ASIO_MOVE_CAST(WriteHandler)(handler));
#else // defined(EGT_ASIO_ENABLE_OLD_SERVICES)
    async_completion<WriteHandler,
      void (egt::asio::error_code, std::size_t)> init(handler);
   ...
   *     egt::asio::buffer(data, size), 0, sender_endpoint, handler); @endcode
   * See the @ref buffer documentation for information on receiving into
   * multiple buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   */
...
#endif // EGT_ASIO_BASIC_RAW_SOCKET_HPP
