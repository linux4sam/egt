...
#ifndef EGT_ASIO_IP_BASIC_RESOLVER_RESULTS_HPP
#define EGT_ASIO_IP_BASIC_RESOLVER_RESULTS_HPP
...
/**
 * The egt::asio::ip::basic_resolver_results class template is used to define
...
public:
  ...
  basic_resolver_results(basic_resolver_results&& other)
    : basic_resolver_iterator<InternetProtocol>(
        EGT_ASIO_MOVE_CAST(basic_resolver_results)(other))
  {
  }
  ...
  static basic_resolver_results create(
      egt::asio::detail::addrinfo_type* address_info,
      const std::string& host_name, const std::string& service_name)
  {
    ...
    while (address_info)
    {
      if (address_info->ai_family == EGT_ASIO_OS_DEF(AF_INET)
          || address_info->ai_family == EGT_ASIO_OS_DEF(AF_INET6))
      {
...
# if defined(EGT_ASIO_WINDOWS_RUNTIME)
  ...
  static basic_resolver_results create(
      ...
      const egt::asio::detail::addrinfo_type& hints,
      const std::string& host_name, const std::string& service_name)
  {
    basic_resolver_results results;
    if (endpoints->Size)
    {
      ...
      for (unsigned int i = 0; i < endpoints->Size; ++i)
      {
        ...
        if (hints.ai_family == EGT_ASIO_OS_DEF(AF_INET)
        ...
        results.values_->push_back(
            basic_resolver_entry<InternetProtocol>(
              typename InternetProtocol::endpoint(
                ip::make_address(
                  egt::asio::detail::winrt_utils::string(
                    pair->RemoteHostName->CanonicalName)),
                egt::asio::detail::winrt_utils::integer(
...
# endif // defined(EGT_ASIO_WINDOWS_RUNTIME)
  ...
  size_type size() const EGT_ASIO_NOEXCEPT
  {
    return this->values_->size();
  }

  /// Get the maximum number of entries permitted in a results range.
  size_type max_size() const EGT_ASIO_NOEXCEPT
  {
    return this->values_->max_size();
  }

  /// Determine whether the results range is empty.
  bool empty() const EGT_ASIO_NOEXCEPT
  {
  ...
  void swap(basic_resolver_results& that) EGT_ASIO_NOEXCEPT
  {
    if (this != &that)
    {
...
#endif // EGT_ASIO_IP_BASIC_RESOLVER_RESULTS_HPP
