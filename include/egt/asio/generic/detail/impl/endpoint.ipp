...
#ifndef EGT_ASIO_GENERIC_DETAIL_IMPL_ENDPOINT_IPP
#define EGT_ASIO_GENERIC_DETAIL_IMPL_ENDPOINT_IPP
...
void endpoint::init(const void* sock_addr,
    std::size_t sock_addr_size, int sock_protocol)
{
  if (sock_addr_size > sizeof(egt::asio::detail::sockaddr_storage_type))
  {
    egt::asio::error_code ec(egt::asio::error::invalid_argument);
    egt::asio::detail::throw_error(ec);
  }
  ...
  memset(&data_.generic, 0, sizeof(egt::asio::detail::sockaddr_storage_type));
  if (sock_addr_size > 0)
...
#endif // EGT_ASIO_GENERIC_DETAIL_IMPL_ENDPOINT_IPP
