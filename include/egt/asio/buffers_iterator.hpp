...
#ifndef EGT_ASIO_BUFFERS_ITERATOR_HPP
#define EGT_ASIO_BUFFERS_ITERATOR_HPP
...
public:
...
#endif // defined(__GNUC__) && (__GNUC__ == 4) && (__GNUC_MINOR__ == 3)
  {
    ...
    new_iter.begin_ = egt::asio::buffer_sequence_begin(buffers);
    new_iter.current_ = egt::asio::buffer_sequence_begin(buffers);
    new_iter.end_ = egt::asio::buffer_sequence_end(buffers);
...
private:
  ...
  void increment()
  {
    EGT_ASIO_ASSERT(current_ != end_ && "iterator out of bounds");
    ++position_;
  ...
  void decrement()
  {
    EGT_ASIO_ASSERT(position_ > 0 && "iterator out of bounds");
    --position_;
  ...
  void advance(std::ptrdiff_t n)
  {
    if (n > 0)
    {
      ...
      for (;;)
      {
        ...
        if (++current_ == end_)
        {
          EGT_ASIO_ASSERT(n == 0 && "iterator out of bounds");
    ...
    {
      std::size_t abs_n = -n;
      EGT_ASIO_ASSERT(position_ >= abs_n && "iterator out of bounds");
      for (;;)
      {
        // Check if the advance can be satisfied by the current buffer.
        ...
        if (current_ == begin_)
        {
          EGT_ASIO_ASSERT(abs_n == 0 && "iterator out of bounds");
          current_buffer_position_ = 0;
          return;
        }
...
#endif // EGT_ASIO_BUFFERS_ITERATOR_HPP
