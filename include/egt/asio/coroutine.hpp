...
#ifndef EGT_ASIO_COROUTINE_HPP
#define EGT_ASIO_COROUTINE_HPP
 ...
 *
 * @code #include <egt/asio/yield.hpp>@endcode
 *
 * and may conversely be undefined as follows:
 *
 * @code #include <egt/asio/unyield.hpp>@endcode
 *
 * <b>reenter</b>
 *
 ...
 *
 * @li @c EGT_ASIO_CORO_REENTER instead of @c reenter
 * @li @c EGT_ASIO_CORO_YIELD instead of @c yield
 * @li @c EGT_ASIO_CORO_FORK instead of @c fork
 */
class coroutine
{
public:
...
#define EGT_ASIO_CORO_REENTER(c) \
  switch (::egt::asio::detail::coroutine_ref _coro_value = c) \
...
#define EGT_ASIO_CORO_YIELD_IMPL(n) \
  for (_coro_value = (n);;) \
    if (_coro_value == 0) \
    { \
      case (n): ; \
...
#define EGT_ASIO_CORO_FORK_IMPL(n) \
...
#if defined(_MSC_VER)
# define EGT_ASIO_CORO_YIELD EGT_ASIO_CORO_YIELD_IMPL(__COUNTER__ + 1)
# define EGT_ASIO_CORO_FORK EGT_ASIO_CORO_FORK_IMPL(__COUNTER__ + 1)
#else // defined(_MSC_VER)
# define EGT_ASIO_CORO_YIELD EGT_ASIO_CORO_YIELD_IMPL(__LINE__)
# define EGT_ASIO_CORO_FORK EGT_ASIO_CORO_FORK_IMPL(__LINE__)
#endif // defined(_MSC_VER)

#endif // EGT_ASIO_COROUTINE_HPP
